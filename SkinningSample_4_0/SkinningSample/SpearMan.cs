...
namespace SmellOfRevenge2011
{
    public class SpearMan
    {
        ...
        public SpearMan(Vector3 pos, Vector3 dir)
        {
        ...
        public void UpdateBattle(GameTime gameTime)
        {
            ...
            if (newMove != null)
            {
                ...
                if (playerMove.Name == "LT")
                {
                    ...
                    ScreenManager.eSpears.Add(new SpearManAI(Vector3.Zero, Vector3.Backward));
                    ScreenManager.eSpears[ScreenManager.eSpears.Count - 1].SkinningData = ScreenManager.spearMan.Tag as SkinningData;
                    ScreenManager.eSpears[ScreenManager.eSpears.Count - 1].setAnimationPlayers();

                }
                if (playerMove.Name == "RT")
                {
                    // resetFormations = true;
                    //  ScreenManager.eSpears[0].state = 1;
                    //  ScreenManager.eSpears[0].pathFinder.Reset();
                    //  ScreenManager.eSpears[0].Moving = false;

                    for (int i = 0; i < ScreenManager.eSpears.Count; i++)
                        ScreenManager.eSpears[i].aiState = 0;
                }
                if (playerMove.Name == "LS")
                {
                    ...
                    for (int i = 0; i < ScreenManager.eSpears.Count; i++)
                        ScreenManager.eSpears[i].aiState = 1;

                   // ScreenManager.battle.Add(new Vector3[45]);

                    if (!ScreenManager.battle)
                    {
                        ScreenManager.battle = true;

                        CFormation.CopyTo(ScreenManager.CFormation, 0);
                        EFormation.CopyTo(ScreenManager.EFormation, 0);
                        WFormation.CopyTo(ScreenManager.WFormation, 0);
                        NFormation.CopyTo(ScreenManager.NFormation, 0);
                        SFormation.CopyTo(ScreenManager.SFormation, 0);

                        formation.Decompose(out scale, out rota, out trans);
                        ScreenManager.battleRota = rota;
                        ScreenManager.battleTrans = trans;
                    }
                }
