...
//float Alpha; //added by nsIII it is in drawwithshadowMap
float4x4 World;
float4x4 View;
float4x4 Projection;
...
DrawNoShadowMap_VSOut DrawNoShadowMap_VertexShader(DrawNoShadowMap_VSIn input)
{
DrawNoShadowMap_VSOut Output;
...
{ 
	...
	///Do this until we add more lights 
    float4 diffuse = diffuseIntensity * diffuseColor + AmbientColor +  ((((diffuseColor / 4))));
    ...
    //diffuse.a = diffuseColor.A;
	diffuse.a = 1.0;
    return diffuse;
}

float4 DrawNoShadowMap_PixelShader(DrawNoShadowMap_VSOut input) : COLOR
{ 
    // Color of the model
    float4 diffuseColor = tex2D(TextureSampler, input.TexCoord);
    // Intensity based on the direction of the light
...
technique SkinnedEffect
{
	pass Pass1
	{
        VertexShader = compile vs_2_0 DrawNoShadowMap_VertexShader();
        PixelShader = compile ps_2_0 DrawNoShadowMap_PixelShader();
	}
