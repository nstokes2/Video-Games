...
float4 PSSkinnedVertexLighting(PSInputTx pin) : SV_Target0
{
	...
	//color = SAMPLE_TEXTURE(Texture, pin.TexCoord);
    return color;
}
...
float4 PSSkinnedVertexLightingNoFog(PSInputTx pin) : SV_Target0
{
	...
	//CHANGED IT TAKE OFF 
	color = SAMPLE_TEXTURE(Texture, pin.TexCoord) * (pin.Diffuse *.8);

	color.a = pin.Diffuse.a;
...
void SkinShadow(inout shadow_INPUT vin, uniform int boneCount)
{
    float4x3 skinning = 0;
...
CreateShadowMap_VSOut CreateShadowMap_VertexShader(shadow_INPUT input)
{
    ...
    Out.Position = mul(input.Position, LightViewProj); 
